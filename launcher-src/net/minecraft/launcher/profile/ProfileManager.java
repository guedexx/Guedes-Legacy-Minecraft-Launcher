...
public class ProfileManager {
	...
	public Profile getSelectedProfile() {
		...
		if (this.selectedProfile != null) {
			selected = this.profiles.get(this.selectedProfile);
		}

		if (selected == null && this.selectedProfile != null) {

			for (Profile profile : this.profiles.values()) {

				if (this.selectedProfile.equals(profile.getName())) {
					selected = profile;
					break;
				}
			}
		...
		if (selected == null) {
			...
			selected = this.profiles.get(DEFAULT_PROFILE_NAME);

			if (selected == null) {

				for (Profile profile : this.profiles.values()) {

					if (DEFAULT_PROFILE_NAME.equals(profile.getName())) {
						selected = profile;
						break;
					}
				}
		...
		if (selected == null && !this.profiles.isEmpty()) {
			selected = this.profiles.values().iterator().next();
		}

		if (selected == null) {

			selected = new Profile(DEFAULT_PROFILE_NAME);
			this.profiles.put(DEFAULT_PROFILE_NAME, selected);
		}

		this.selectedProfile = selected.getName();

		try {

			InputStream in = Launcher.class.getResourceAsStream(
					selected.squidHQMode()
							? "/squidhq.png"
							: "/favicon.png"
			);

			Launcher.getCurrentInstance()
					.getFrame()
					.setIconImage(ImageIO.read(in));

			Launcher.getCurrentInstance().squidHQ =
					selected.squidHQMode();

		} catch (IOException e) {

			LogManager.getLogger()
					.error("Couldn't update frame icon.");
		}
	...
	public void setSelectedProfile(String selectedProfile) {
		...
		boolean update = !Objects.equal(this.selectedProfile, selectedProfile);
