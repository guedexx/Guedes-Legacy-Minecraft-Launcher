	...
	public boolean appliesToCurrentEnvironment(
		...
		if (
				this.compatibilityRules == null
						|| this.compatibilityRules.isEmpty()
		) {
			return true;
		}

		CompatibilityRule.Action lastAction =
				CompatibilityRule.Action.DISALLOW;
		...
		public Argument deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) throws JsonParseException {
			...
			if (json.isJsonObject()) {
				...
				if (obj.has("rules")) {
					...
					rules =
							new ArrayList<CompatibilityRule>();

					JsonArray array =
							obj.getAsJsonArray("rules");
