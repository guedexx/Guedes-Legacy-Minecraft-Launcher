...
public class CompleteMinecraftVersion implements Version {
		...
		if (version.arguments != null) {
			...
			for (Map.Entry<ArgumentType, List<Argument>> entry : version.arguments.entrySet()) {
				...
				if (entry.getKey() != null) {
					this.arguments.put(
							entry.getKey(),
							new ArrayList<Argument>(entry.getValue())
					);
				}
			}
		} 
	...
	public Collection<File> getClassPath(
			OperatingSystem os,
			File base,
			CompatibilityRule.FeatureMatcher featureMatcher
	) {

		Collection<Library> libraries =
				getRelevantLibraries(featureMatcher);

		Set<File> result =
				new java.util.LinkedHashSet<File>();
		...
		File preferredAsm = null;

		for (File file : result) {
			String path = file.getAbsolutePath().replace('\\', '/');

			if (
					path.contains("/org/ow2/asm/asm/")
							&& file.getName().equals("asm-9.7.1.jar")
			) {
				preferredAsm = file;
				break;
			}
		...
		if (preferredAsm != null) {
			final File finalPreferredAsm = preferredAsm;

			result.removeIf(file -> {
				String path =
						file.getAbsolutePath().replace('\\', '/');

				return path.contains("/org/ow2/asm/asm/")
						&& file.getName().startsWith("asm-")
						&& !file.equals(finalPreferredAsm);
			});
		}

		System.out.println("========== ASM CLASSPATH ==========");
