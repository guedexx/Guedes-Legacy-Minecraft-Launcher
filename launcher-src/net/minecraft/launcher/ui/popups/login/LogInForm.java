...
public class LogInForm extends JPanel implements ActionListener {
	...
	public void tryLogIn() {
		...
		if (this.authentication.isLoggedIn() && this.authentication.getSelectedProfile() == null && ArrayUtils.isNotEmpty((Object[])this.authentication.getAvailableProfiles())) {
			...
			final int passwordLength = passwordChars.length;

			if (!(this.authentication instanceof YggdrasilUserAuthentication)) {
				this.authentication.setPassword(String.valueOf(passwordChars));
			}
			...
			this.popup.getMinecraftLauncher().getLauncher().getVersionManager().getExecutorService().execute(new Runnable() {
				...
				public void run() {
					...
					try {
						...
						if (LogInForm.this.authentication instanceof YggdrasilUserAuthentication) {

							((YggdrasilUserAuthentication) LogInForm.this.authentication)
									.logInWithMicrosoft();
						...
						if (LogInForm.this.authentication.getSelectedProfile() == null) {
							...
							authDatabase.register(
									UUIDTypeAdapter.fromUUID(
											LogInForm.this.authentication.getSelectedProfile().getId()
									),
							...
							try {
								...
								LogInForm.LOGGER.info("Microsoft session saved");

							} catch (IOException ex) {

								LogInForm.LOGGER.error(
										"Couldn't save Microsoft session",
										ex
								);
							}

							LogInForm.this.popup.setLoggedIn(
									UUIDTypeAdapter.fromUUID(
											LogInForm.this.authentication.getSelectedProfile().getId()
									)
							);
