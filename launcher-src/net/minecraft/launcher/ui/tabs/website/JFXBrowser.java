		...
		Platform.runLater(new Runnable() {
			...
			public void run() {
				...
				synchronized (JFXBrowser.this.lock) {
					...
					JFXBrowser.this.webEngine.setJavaScriptEnabled(true);
								...
								if (doc != null) {
									...
									for (int i = 0; i < images.getLength(); i++) {
										...
										if (node instanceof Element) {
											...
											if (
													(src != null && src.toLowerCase().contains("cobalt")) ||
															(alt != null && alt.toLowerCase().contains("cobalt"))
											) {

												try {

													java.io.InputStream input =
															JFXBrowser.class.getResourceAsStream("/cobalt_logo_150.png");
													...
													if (input != null) {
														...
														JFXBrowser.LOGGER.info("Cobalt logo replaced successfully");

													} else {

														JFXBrowser.LOGGER.error(
																"Could not find /cobalt_logo_150.png inside launcher JAR"
														);
													}
